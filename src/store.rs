...
pub fn store(props: &Props) -> Html{
    ...
    let bux = props.bux.clone();
    let sock_bought = props.sock_bought.clone();
    let onclick_common = move|_|if COMMON <= bux {sock_bought.emit(Msg::BoughtCommon)};

    let bux = props.bux.clone();
    let sock_bought = props.sock_bought.clone();
    let onclick_rare = move|_|if RARE <= bux {sock_bought.emit(Msg::BoughtRare)};

    let bux = props.bux.clone();
    let sock_bought = props.sock_bought.clone();
    let onclick_legend = move|_|if LEGEND <= bux {sock_bought.emit(Msg::BoughtLegend)};
